...
def test_multitarget_macenko_torch():
    ...
    # initialize normalizers for each backend and fit to target image
    normalizer = torchstain.normalizers.MultiMacenkoNormalizer(backend="numpy", norm_mode="avg-post")
    normalizer.fit(target)

    torch_normalizer = torchstain.normalizers.MultiMacenkoNormalizer(backend="torch", norm_mode="avg-post")
    torch_normalizer.fit(T(target))
